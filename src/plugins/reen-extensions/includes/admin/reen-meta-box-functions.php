<?php
/**
 * Reen Meta Box Functions
 *
 * @author      MadrasThemes
 * @category    Core
 * @package     ReenExtensions/Admin/Functions
 * @version     1.0.0
 */
if ( ! defined( 'ABSPATH' ) ) {
 ...
 * @param array $field
 */
function reen_wp_text_input( $field ) {
    ...
    switch ( $data_type ) {
        case 'decimal':
            ...
            $field['value']  = reen_format_localized_decimal( $field['value'] );
            break;
        case 'url':
    ...
    if ( ! empty( $field['description'] ) && false !== $field['desc_tip'] ) {
        echo reen_help_tip( $field['description'] );
    }
 ...
 * @param array $field
 */
function reen_wp_hidden_input( $field ) {
    global $thepostid, $post;
 ...
 * @param array $field
 */
function reen_wp_textarea_input( $field ) {
    global $thepostid, $post;
 ...
 * @param array $field
 */
function reen_wp_checkbox( $field ) {
    global $thepostid, $post;
...
function reen_wp_select( $field ) {
    ...
    ?>
    <p <?php echo reen_implode_html_attributes( $wrapper_attributes ); // WPCS: XSS ok. ?>>
        <label <?php echo reen_implode_html_attributes( $label_attributes ); // WPCS: XSS ok. ?>><?php echo wp_kses_post( $field['label'] ); ?></label>
        <?php if ( $tooltip ) : ?>
            <?php echo reen_help_tip( $tooltip ); // WPCS: XSS ok. ?>
        <?php endif; ?>
        <select <?php echo reen_implode_html_attributes( $field_attributes ); // WPCS: XSS ok. ?>>
            <?php
            foreach ( $field['options'] as $key => $value ) {
                echo '<option value="' . esc_attr( $key ) . '"' . reen_selected( $key, $field['value'] ) . '>' . esc_html( $value ) . '</option>';
            }
            ?>
        </select>
        <?php if ( $description ) : ?>
